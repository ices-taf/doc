...
See also: [:arrow_forward: TAF video](https://www.youtube.com/watch?v=FweJbr9hfdY),
[:bookmark_tabs: TAF procedure](../procedure.md)
...
using `cp` and `mkdir` to manipulate files. We used `tafpng` to open a PNG
...
example, in `report.R` we were using `tafpng` so it uses the suggested image
size and so forth.
